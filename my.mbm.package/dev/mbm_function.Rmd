...
model_based_extraction <- function(weights, sd_diff = 0 , min_diff=2 , model_backend="rstan"){
  ...
  fit.normal <- my.mbm.package::fit_stan_model(normal_model_base,'weights ~ 1', data.frame(weights=weights), family="normal")
  
  #fit a model with a skew normal distribution
  fit.skew <- my.mbm.package::fit_stan_model (skewnormal_model_base,'weights ~ 1', data.frame(weights=weights), family="skew_normal")
  ...
  if(rownames(loo_res)[1]=="fit.skew" & (loo_res[2,1] + sd_diff*loo_res[2,2] < 0 ) & (loo_res[2,1]<=(-min_diff))  ){  #   min_diff makes it more conservative...
    ...
    for (s in 0:round(length(weights)/2) ){ #only look at the highest half of the weights
      ...
      if(s==0){
        ...
        fit.skew2 <-my.mbm.package::fit_stan_model(poisson_model_base,'weights ~ 1', data.frame(weights=weights), family="poisson")
        ...
        #fit a skewed normal
        fit.skew2 <- my.mbm.package::fit_stan_model(poisson_model_label,'weights ~ 1 + label', data.frame(weights=weights, label=as.factor(weight_labels) ), family="poisson")
...
system("git config --global user.name 'Chloe Vilette'")
system("git config --global user.email 'c.vilette@uleth.ca'")
